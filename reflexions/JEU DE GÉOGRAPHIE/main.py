...
def random_correct_city():
    ...
    reponse = requests.get('https://geo.api.gouv.fr/communes?code=' + str(code_postal) + '&fields=nom,centre,population').json()
    while reponse == []:
        code_postal=random_code_postal()
        reponse = requests.get('https://geo.api.gouv.fr/communes?code=' + str(code_postal) + '&fields=nom,centre,population').json()
    print(reponse)
...
def afficher_ville(ville):
    response = requests.get('https://geo.api.gouv.fr/communes?nom=' + ville + '&fields=nom,centre,population')
...
def main(GAMES=20,MODE=3, VILLE = 'Vesdun'):
    ...
    if MODE == 1:
        background = pygame.image.load("img/fonddecarte.jpg")
    else:
        background = pygame.image.load("img/fonddecarte.jpg")
